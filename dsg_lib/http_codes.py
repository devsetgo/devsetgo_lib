...
The dictionary `ALL_HTTP_CODES` contains the HTTP status codes as keys. Each key maps to another dictionary that contains a description of the status code and a link to its documentation on the Mozilla Developer Network (MDN).

Example:
    ```python
    from dsg_lib import http_codes

    # Get the description and link for HTTP status code 200
...
Variables:
    ALL_HTTP_CODES (dict): A dictionary of HTTP status codes. Each key is an HTTP status code (int), and each value is another dictionary with keys 'description' (str) and 'link' (str).
"""
...
def generate_code_dict(codes, description_only=False):
    ...
    Args:
        ...
        description_only (bool, optional): If True, only the description of the codes will be returned. Defaults to False.

    Returns:
        dict: A dictionary where each key is an HTTP error code from the input list and each value depends on the description_only parameter.
    ...
    Example:
        ...
        print(status_dict)  # {200: {'description': 'OK', 'link': 'https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/200'}, 404: {'description': 'Not Found', 'link': 'https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/404'}}
...
# Usage:
# A list of common HTTP status codes used in various HTTP methods
...
# A dictionary of common HTTP status codes and additional codes specific to POST requests
...
# A dictionary of common HTTP status codes and additional codes specific to PUT requests
...
# A dictionary of common HTTP status codes and additional codes specific to PATCH requests
...
# A dictionary of common HTTP status codes and additional codes specific to DELETE requests
